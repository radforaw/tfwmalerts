...
class incidentdatabase(): #requestscache
	...
	def queryclean(self):
		self.c.execute('''UPDATE incidents SET endtime=? WHERE (endtime=? AND (?-starttime)>86400)''',(time.time(),-1,time.time()))
		self.conn.commit()
		
	def resetclean(self):
		self.c.execute('''UPDATE incidents SET endtime=?, info=? WHERE (endtime=?)''',(time.time(),"closed due to system restart",-1))
		self.conn.commit()
